...
int ssh_get_pubkey_hash(ssh_session session, unsigned char **hash)
{
    ...
    *hash = NULL;
    ...
    md5_update_esp32_port(ctx, ssh_string_data(pubkey_blob), ssh_string_len(pubkey_blob));
    ...
    case SSH_PUBLICKEY_HASH_SHA1:
        {
            ...
            sha1_update_esp32_port(ctx, ssh_string_data(blob), ssh_string_len(blob));
            sha1_final(h, ctx);
    ...
    case SSH_PUBLICKEY_HASH_MD5:
        {
            ...
            md5_update_esp32_port(ctx, ssh_string_data(blob), ssh_string_len(blob));
            md5_final(h, ctx);
